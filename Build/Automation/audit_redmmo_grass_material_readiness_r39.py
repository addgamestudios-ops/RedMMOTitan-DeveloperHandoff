"""Read-only R39 exact grass-material graph and D3D12 readiness audit."""

from __future__ import annotations

import hashlib
import json
import math
import os
import socket
import time
import traceback
from datetime import datetime, timezone
from pathlib import Path

import unreal


PROJECT = Path(r"D:\RedMMOTitanWindowsData\Projects\RedMMO\RedMMO.uproject")
PROJECT_SHA = "3BF3D8D1D1C7F892A2CD4873F7C0390EC1AF3CA8F77E60549169563D18DBF86F"
HOME_MAP = "/Game/RedMMO/Maps/RedMMO_PPG_HomeWorld"
HOME_FILE = PROJECT.parent / r"Content\RedMMO\Maps\RedMMO_PPG_HomeWorld.umap"
HOME_SHA = "0331E54155DF929C82E56084BEAD569865052A25D20A6082BA9C3E07E88F88F3"
PROFILE_FILE = PROJECT.parent / r"Content\RedMMO\WorldAuthoring\PPG\ProfileV1\DA_PPG_ProfileV1_PlanetData.uasset"
PROFILE_SHA = "D226215C7367808F4A2E3225A0C9CBD7F4F32E803ABEFD22CA39062CD5538970"
FOLIAGE_FILE = PROJECT.parent / r"Content\RedMMO\WorldAuthoring\PPG\ProfileV1\Profiles\DA_PPG_ProfileV1_GrassEligible_R29.uasset"
FOLIAGE_SHA = "D1ACEE4F403D2082FF49CB57D52907AAADA0C20D06386576C435557CF49404D8"
SURFACE_FILE = PROJECT.parent / r"Content\RedMMO\WorldAuthoring\PPG\ProfileV1\MI_PPG_ProfileV1_Surface.uasset"
SURFACE_SHA = "FBB3A58782DEED69D93BB2369387873FA2DA7FEAA93F47CBA289FC61EBEFBD66"
PARENT = "/Game/RedMMO/World/PPG/HomeWorld/StylizedBinding/R32/Materials/M_GrassChunks_PPGReadable_R32"
PARENT_FILE = PROJECT.parent / r"Content\RedMMO\World\PPG\HomeWorld\StylizedBinding\R32\Materials\M_GrassChunks_PPGReadable_R32.uasset"
PARENT_SHA = "2BD2B8DD41C611CF1250F1A39C40D3B4A7C47B5EB71ECD1497732546B80F0210"
MATERIALS = {
    "/Game/RedMMO/World/PPG/HomeWorld/StylizedBinding/R10N/Materials/MI_GrassChunks_DenseTall_A_R10N": (
        PROJECT.parent / r"Content\RedMMO\World\PPG\HomeWorld\StylizedBinding\R10N\Materials\MI_GrassChunks_DenseTall_A_R10N.uasset",
        "91A0E7233A5922A921FB4CF8692B8631DBB7AEFE7D3D106FDB63C44DB412CE47",
    ),
    "/Game/RedMMO/World/PPG/HomeWorld/StylizedBinding/R10N/Materials/MI_GrassChunks_DenseTall_B_R10N": (
        PROJECT.parent / r"Content\RedMMO\World\PPG\HomeWorld\StylizedBinding\R10N\Materials\MI_GrassChunks_DenseTall_B_R10N.uasset",
        "19D1B594553977A6A7BA116271F103DCD4044F8F24C3E8B5CF8F596082A9F68E",
    ),
}
MESHES = {
    "/Game/RedMMO/World/PPG/HomeWorld/StylizedBinding/R10N/Meshes/SM_GrassChunk_DenseTall_A_R10N",
    "/Game/RedMMO/World/PPG/HomeWorld/StylizedBinding/R10N/Meshes/SM_GrassChunk_DenseTall_B_R10N",
}
PROTECTED = {
    Path(r"D:\RedMMOTitan\Content\RedMMO\Maps\RedPlanetGen_50km_Test.umap"): "DA5987012EEB8F550254F5FCF0F47F42B8F5E03428CF4881C83D8687537F6E7D",
    Path(r"D:\RedMMOTitan\Content\RedMMO\Maps\RedPlanetGen_50km_FusedPrototype.umap"): "4C84D7752C87854C70987ED9988C64C124C577FBE1DE3DEF3CC826F81B0B6284",
}
DIAG = Path(r"D:\RedMMOTitanWindowsData\Diagnostics\RedMMO_GrassMaterialReadiness_R39_20260805T1450Z")
RESULT = DIAG / "result.json"


def require(condition, message):
    if not condition:
        raise RuntimeError(message)


def now():
    return datetime.now(timezone.utc).isoformat()


def sha256(path):
    digest = hashlib.sha256()
    with Path(path).open("rb") as stream:
        for block in iter(lambda: stream.read(1024 * 1024), b""):
            digest.update(block)
    return digest.hexdigest().upper()


def atomic_json(path, payload):
    path.parent.mkdir(parents=True, exist_ok=True)
    data = (json.dumps(payload, indent=2, sort_keys=True, default=str) + "\n").encode("utf-8")
    with path.open("xb") as stream:
        stream.write(data)
        stream.flush()
        os.fsync(stream.fileno())


def provider_gate():
    state = {}
    for port in (11111, 5353, 8000, 8765):
        probe = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        probe.settimeout(0.15)
        try:
            state[str(port)] = probe.connect_ex(("127.0.0.1", port)) != 0
        finally:
            probe.close()
    require(all(state.values()), "provider listener active: " + repr(state))
    return state


def asset_path(value):
    if value is None:
        return None
    path = value.get_path_name().split(":", 1)[0]
    leaf = path.rsplit("/", 1)[-1]
    return path.rsplit(".", 1)[0] if "." in leaf else path


def dirty_packages():
    return {
        "content": sorted({asset_path(item) for item in unreal.EditorLoadingAndSavingUtils.get_dirty_content_packages()}),
        "maps": sorted({asset_path(item) for item in unreal.EditorLoadingAndSavingUtils.get_dirty_map_packages()}),
    }


def normalized(value):
    return str(value).replace("_", "").replace(" ", "").lower()


def struct_field(value, wanted):
    fields = value.to_dict()
    target = normalized(wanted)
    matches = [item for key, item in fields.items() if normalized(key) in (target, "b" + target)]
    require(len(matches) == 1, "expected reflected field {} in {}".format(wanted, list(fields)))
    return matches[0]


def color(value):
    return [float(value.r), float(value.g), float(value.b), float(value.a)]


def safe_property(obj, name):
    try:
        value = obj.get_editor_property(name)
        if hasattr(value, "get_path_name"):
            return asset_path(value)
        if hasattr(value, "to_dict"):
            return value.to_dict()
        return str(value)
    except Exception as error:
        return {"unavailable": str(error)}


def expression_record(expression):
    record = {
        "class": expression.get_class().get_name(),
        "path": expression.get_path_name(),
    }
    for prop in ("parameter_name", "texture", "value", "default_value", "desc"):
        value = safe_property(expression, prop)
        if not (isinstance(value, dict) and "unavailable" in value):
            record[prop] = value
    try:
        record["input_names"] = [str(item) for item in unreal.MaterialEditingLibrary.get_material_expression_input_names(expression)]
    except Exception as error:
        record["input_names_error"] = str(error)
    return record


def material_property_record(material, enum_name):
    record = {"enum": enum_name}
    enum_value = getattr(unreal.MaterialProperty, enum_name, None)
    record["enum_available"] = enum_value is not None
    if enum_value is None:
        return record
    library = unreal.MaterialEditingLibrary
    record["input_api_available"] = hasattr(library, "get_material_property_input_node")
    if record["input_api_available"]:
        try:
            node = library.get_material_property_input_node(material, enum_value)
            record["connected_node"] = expression_record(node) if node is not None else None
        except Exception as error:
            record["connected_node_error"] = str(error)
    record["output_name_api_available"] = hasattr(library, "get_material_property_input_node_output_name")
    if record["output_name_api_available"]:
        try:
            record["connected_output"] = str(library.get_material_property_input_node_output_name(material, enum_value))
        except Exception as error:
            record["connected_output_error"] = str(error)
    return record


def instance_record(path):
    instance = unreal.load_asset(path)
    require(instance is not None, "material instance missing: " + path)
    library = unreal.MaterialEditingLibrary
    scalars = {}
    vectors = {}
    textures = {}
    switches = {}
    for name in [str(item) for item in library.get_scalar_parameter_names(instance)]:
        try:
            scalars[name] = float(library.get_material_instance_scalar_parameter_value(instance, name))
        except Exception as error:
            scalars[name] = {"error": str(error)}
    for name in [str(item) for item in library.get_vector_parameter_names(instance)]:
        try:
            vectors[name] = color(library.get_material_instance_vector_parameter_value(instance, name))
        except Exception as error:
            vectors[name] = {"error": str(error)}
    get_texture_names = getattr(library, "get_texture_parameter_names", None)
    get_texture_value = getattr(library, "get_material_instance_texture_parameter_value", None)
    if get_texture_names is not None and get_texture_value is not None:
        for name in [str(item) for item in get_texture_names(instance)]:
            try:
                textures[name] = asset_path(get_texture_value(instance, name))
            except Exception as error:
                textures[name] = {"error": str(error)}
    for name in [str(item) for item in library.get_static_switch_parameter_names(instance)]:
        try:
            switches[name] = bool(library.get_material_instance_static_switch_parameter_value(instance, name))
        except Exception as error:
            switches[name] = {"error": str(error)}
    return {
        "path": path,
        "parent": asset_path(instance.get_editor_property("parent")),
        "base_material": asset_path(instance.get_base_material()),
        "scalar_parameters": scalars,
        "vector_parameters": vectors,
        "texture_parameters": textures,
        "static_switch_parameters": switches,
    }


def compiling_record():
    result = {
        "asset_compiling_manager_class_available": hasattr(unreal, "AssetCompilingManager"),
        "shader_compiling_manager_class_available": hasattr(unreal, "ShaderCompilingManager"),
        "material_editing_compile_related_api": sorted(
            name for name in dir(unreal.MaterialEditingLibrary) if "compil" in name.lower()
        ),
    }
    manager_class = getattr(unreal, "AssetCompilingManager", None)
    if manager_class is not None:
        try:
            manager = manager_class.get()
            result["asset_compiling_manager_get_succeeded"] = manager is not None
            for name in ("get_num_remaining_assets", "get_num_remaining_jobs", "get_num_remaining_assets_by_type"):
                method = getattr(manager, name, None)
                if method is not None:
                    try:
                        result[name] = method()
                    except Exception as error:
                        result[name + "_error"] = str(error)
        except Exception as error:
            result["asset_compiling_manager_get_error"] = str(error)
    return result


class R39:
    def __init__(self):
        self.editor = unreal.get_editor_subsystem(unreal.UnrealEditorSubsystem)
        self.level = unreal.get_editor_subsystem(unreal.LevelEditorSubsystem)
        self.handle = None
        self.phase = "PREPARE"
        self.started = time.monotonic()
        self.world = None
        self.pawn = None
        self.spawner = None
        self.report = {
            "schema": "redmmo.grass_material_readiness.audit.r39.v1",
            "status": "RUNNING",
            "started_utc": now(),
            "evidence_class": "automation",
        }

    def set_phase(self, phase):
        self.phase = phase
        self.started = time.monotonic()
        self.report["phase"] = phase
        unreal.log("REDMMO_R39_PHASE " + phase)

    def authenticate(self):
        active = Path(unreal.Paths.convert_relative_path_to_full(unreal.Paths.get_project_file_path())).resolve(strict=True)
        require(active == PROJECT.resolve(strict=True), "wrong project")
        require(not RESULT.exists(), "R39 no-clobber failed")
        for path, expected in (
            (PROJECT, PROJECT_SHA), (HOME_FILE, HOME_SHA), (PROFILE_FILE, PROFILE_SHA),
            (FOLIAGE_FILE, FOLIAGE_SHA), (SURFACE_FILE, SURFACE_SHA), (PARENT_FILE, PARENT_SHA),
        ):
            require(path.is_file() and sha256(path) == expected, "input drift: " + str(path))
        for path, expected in list(MATERIALS.values()) + list(PROTECTED.items()):
            require(path.is_file() and sha256(path) == expected, "material/protected drift: " + str(path))
        require(dirty_packages() == {"content": [], "maps": []}, "editor started dirty")
        require(not self.level.is_in_play_in_editor(), "PIE already active")
        command = str(unreal.SystemLibrary.get_command_line()).lower()
        require("-d3d12" in command and "-renderoffscreen" in command and "-nullrhi" not in command, "renderer gate failed")
        self.report["command_line"] = command
        self.report["provider_gate_before"] = provider_gate()
        world = unreal.EditorLevelLibrary.get_editor_world()
        require(world is not None and world.get_path_name().split(":", 1)[0].split(".", 1)[0] == HOME_MAP, "wrong map")
        parent = unreal.load_asset(PARENT)
        require(parent is not None, "R32 parent failed to load")
        expressions = []
        try:
            expressions = [expression_record(item) for item in unreal.MaterialEditingLibrary.get_material_expressions(parent)]
        except Exception as error:
            self.report["expression_enumeration_error"] = str(error)
        self.report["parent_material"] = {
            "path": PARENT,
            "blend_mode": str(parent.get_editor_property("blend_mode")),
            "opacity_mask_clip_value": float(parent.get_editor_property("opacity_mask_clip_value")),
            "two_sided": bool(parent.get_editor_property("two_sided")),
            "base_color": material_property_record(parent, "MP_BASE_COLOR"),
            "opacity_mask": material_property_record(parent, "MP_OPACITY_MASK"),
            "normal": material_property_record(parent, "MP_NORMAL"),
            "expressions": expressions,
        }
        self.report["instances"] = [instance_record(path) for path in sorted(MATERIALS)]
        self.report["compile_state_before_pie"] = compiling_record()
        require(dirty_packages() == {"content": [], "maps": []}, "graph audit dirtied packages")
        self.level.editor_request_begin_play()
        self.set_phase("WAIT_PIE")

    def bind_pie(self):
        world = self.editor.get_game_world()
        if world is None or not self.level.is_in_play_in_editor():
            return False
        pawn = unreal.GameplayStatics.get_player_pawn(world, 0)
        spawners = [actor for actor in unreal.GameplayStatics.get_all_actors_of_class(world, unreal.Actor) if actor.get_class().get_name() == "PlanetSpawnerBP_C"]
        if pawn is None or len(spawners) != 1:
            return False
        self.world = world
        self.pawn = pawn
        self.spawner = spawners[0]
        self.set_phase("WAIT_GENERATION")
        return True

    def generation_ready(self):
        status = self.spawner.get_planet_generation_status()
        record = {
            "phase": str(status.get_editor_property("phase")),
            "progress": float(status.get_editor_property("progress")),
            "is_generating": bool(status.get_editor_property("is_generating")),
        }
        self.report["generation"] = record
        return "COMPLETE" in record["phase"].upper() and record["progress"] >= 0.999 and not record["is_generating"]

    def collect_runtime(self):
        foliage_actor = self.spawner.get_foliage_actor()
        require(foliage_actor is not None, "foliage actor missing")
        components = [
            component for component in list(foliage_actor.get_components_by_class(unreal.StaticMeshComponent))
            if component.get_class().get_name() == "PPGGPUFoliageComponent"
            and asset_path(component.get_editor_property("static_mesh")) in MESHES
        ]
        require(len(components) == 196, "approved component-count drift")
        records = []
        for component in components:
            diag = unreal.RedPPGFoliageDiagnostics.inspect_component(component)
            material = component.get_material(0)
            records.append({
                "mesh": asset_path(component.get_editor_property("static_mesh")),
                "material": asset_path(material),
                "base_material": asset_path(material.get_base_material()) if material is not None else None,
                "instances": int(struct_field(diag, "num_instances")),
                "registered": bool(struct_field(diag, "registered")),
                "render_state_created": bool(struct_field(diag, "render_state_created")),
                "has_scene_proxy": bool(struct_field(diag, "has_scene_proxy")),
                "last_render_time_on_screen": float(struct_field(diag, "last_render_time_on_screen")),
            })
        materials = {}
        bases = {}
        for item in records:
            materials[item["material"]] = materials.get(item["material"], 0) + 1
            bases[item["base_material"]] = bases.get(item["base_material"], 0) + 1
        self.report["runtime"] = {
            "components": len(records),
            "instances": sum(item["instances"] for item in records),
            "registered": sum(item["registered"] for item in records),
            "render_state_created": sum(item["render_state_created"] for item in records),
            "scene_proxy_present": sum(item["has_scene_proxy"] for item in records),
            "positive_last_render_time": sum(item["last_render_time_on_screen"] > 0.0 for item in records),
            "materials": materials,
            "base_materials": bases,
        }
        self.report["compile_state_after_settle"] = compiling_record()
        self.level.editor_request_end_play()
        self.set_phase("WAIT_STOP")

    def finish(self):
        require(not self.level.is_in_play_in_editor(), "PIE did not stop")
        require(dirty_packages() == {"content": [], "maps": []}, "PIE dirtied packages")
        for path, expected in (
            (HOME_FILE, HOME_SHA), (PROFILE_FILE, PROFILE_SHA), (FOLIAGE_FILE, FOLIAGE_SHA),
            (SURFACE_FILE, SURFACE_SHA), (PARENT_FILE, PARENT_SHA),
        ):
            require(sha256(path) == expected, "post-PIE drift: " + str(path))
        for path, expected in list(MATERIALS.values()) + list(PROTECTED.items()):
            require(sha256(path) == expected, "material/protected post-drift: " + str(path))
        opacity = self.report["parent_material"]["opacity_mask"]
        base = self.report["parent_material"]["base_color"]
        self.report.update({
            "status": "PASS_R39_EXACT_GRASS_MATERIAL_AND_READINESS_AUDIT",
            "completed_utc": now(),
            "diagnosis": {
                "base_color_connected": base.get("connected_node") is not None,
                "opacity_mask_connected": opacity.get("connected_node") is not None,
                "runtime_exact_materials_bound": set(self.report["runtime"]["materials"]) == set(MATERIALS),
                "runtime_exact_parent_bound": set(self.report["runtime"]["base_materials"]) == {PARENT},
            },
            "dirty_packages_after": dirty_packages(),
            "provider_gate_after": provider_gate(),
            "save_called": False,
            "screenshot_called": False,
            "claim_limit": "Fresh D3D12 PIE material-graph/readiness automation only; no save, screenshot, visual acceptance, gameplay, package or multiplayer claim.",
        })
        atomic_json(RESULT, self.report)
        unreal.log("REDMMO_R39_PASS")
        self.phase = "DONE"
        self.schedule_quit(3.0)

    def fail(self, error):
        failed_phase = self.phase
        self.phase = "FAILED"
        self.report.update({"status": "FAIL", "failed_phase": failed_phase, "completed_utc": now(), "error": str(error), "traceback": traceback.format_exc()})
        if self.level.is_in_play_in_editor():
            self.level.editor_request_end_play()
        if not RESULT.exists():
            atomic_json(RESULT, self.report)
        unreal.log_error("REDMMO_R39_FAIL " + str(error))
        self.schedule_quit(2.0)

    def schedule_quit(self, delay):
        started = time.monotonic()
        old = self.handle
        if old is not None:
            try:
                unreal.unregister_slate_post_tick_callback(old)
            except Exception:
                pass

        def tick(_delta):
            if time.monotonic() - started < delay:
                return
            try:
                unreal.unregister_slate_post_tick_callback(self.handle)
            except Exception:
                pass
            unreal.SystemLibrary.quit_editor()

        self.handle = unreal.register_slate_post_tick_callback(tick)

    def tick(self, _delta):
        try:
            elapsed = time.monotonic() - self.started
            if self.phase == "PREPARE":
                self.authenticate()
            elif self.phase == "WAIT_PIE":
                require(elapsed <= 25.0, "PIE startup timeout")
                self.bind_pie()
            elif self.phase == "WAIT_GENERATION":
                require(elapsed <= 240.0, "generation timeout")
                if self.generation_ready():
                    self.set_phase("SETTLE")
            elif self.phase == "SETTLE":
                require(elapsed <= 30.0, "runtime settle timeout")
                if elapsed >= 10.0:
                    self.collect_runtime()
            elif self.phase == "WAIT_STOP":
                require(elapsed <= 15.0, "PIE stop timeout")
                if not self.level.is_in_play_in_editor():
                    self.finish()
        except Exception as error:
            self.fail(error)

    def start(self):
        self.handle = unreal.register_slate_post_tick_callback(self.tick)


try:
    _R39 = R39()
    _R39.start()
except Exception as bootstrap_error:
    atomic_json(RESULT, {"schema": "redmmo.grass_material_readiness.audit.r39.v1", "status": "FAIL", "completed_utc": now(), "error": str(bootstrap_error), "traceback": traceback.format_exc()})
    unreal.SystemLibrary.quit_editor()
