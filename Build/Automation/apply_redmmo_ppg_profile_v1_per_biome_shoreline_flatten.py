"""Persist the authenticated six-branch shoreline flatten design in ProfileV1.

Only the unbound ProfileV1 generation material may change. One shared scalar
feeds six native PPG flatten nodes, each wrapping the already-connected height
source for its named biome output. Global Height remains unconnected. No home
map binding, generation, map save, or visual claim is permitted.
"""
...
DIAG = Path(r"D:\RedMMOTitanWindowsData\Diagnostics\RedMMO_PPG_ProfileV1PerBiomeShorelineFlatten_20260805_0541")
RESULT = DIAG / "apply_redmmo_ppg_profile_v1_per_biome_shoreline_flatten_result.json"
...
def require_unchanged_hashes():
    ...
    for path, expected_hash in EXPECTED_UNCHANGED.items():
        require(asset_file(path).is_file() and sha256(asset_file(path)) == expected_hash, "Unchanged ProfileV1 hash drift: " + path)
...
def main():
    report = {
        "schema": "redmmo.ppg_profile_v1.per_biome_shoreline_flatten.build.v1",
        "status": "RUNNING",
        "started_utc": now(),
    ...
    try:
        ...
        nodes_before = list(unreal.MaterialEditingLibrary.get_material_expressions(material))
        require(not [node for node in nodes_before if node.get_class().get_name() == "MaterialExpressionPlanetFlattenElevation"], "Native flatten node already exists")
        require(not [
        ...
        ], "ShorelineFlattenThreshold already exists")
        ...
        output = outputs[0]
        before_inputs = input_sources(material, output)
        require(before_inputs.get("Global Height") is None, "Global Height must remain unconnected")
        original_sources = {}
        for role, expected_source_name in ROLE_SOURCES:
            input_name = role + " Height"
            source = before_inputs.get(input_name)
            require(source is not None, "Missing biome height source: " + role)
            require(source.get_name() == expected_source_name, "Biome height source drift: " + role)
            original_sources[role] = source

        threshold = unreal.MaterialEditingLibrary.create_material_expression(
            material, unreal.MaterialExpressionScalarParameter, -300, -1500
        )
        require(threshold is not None, "Threshold expression creation failed")
        ...
        wrappers = []
        for index, (role, _expected_source_name) in enumerate(ROLE_SOURCES):
            input_name = role + " Height"
            flatten = unreal.MaterialEditingLibrary.create_material_expression(
                material, unreal.MaterialExpressionPlanetFlattenElevation, 40, -1300 + index * 260
            )
            require(flatten is not None, "Flatten expression creation failed: " + role)
            flatten.set_editor_property("water_level", 0.0)
            flatten.set_editor_property("threshold", 0.005)
            flatten.set_editor_property("desc", "RedProfile.ShorelineFlatten." + role)
            require(unreal.MaterialEditingLibrary.disconnect_material_expressions(output, input_name), "Could not disconnect " + input_name)
            require(unreal.MaterialEditingLibrary.connect_material_expressions(original_sources[role], "", flatten, "Elevation"), "Could not connect Elevation: " + role)
            require(unreal.MaterialEditingLibrary.connect_material_expressions(threshold, "", flatten, "Threshold"), "Could not connect Threshold: " + role)
            require(unreal.MaterialEditingLibrary.connect_material_expressions(flatten, "", output, input_name), "Could not connect output: " + role)
            wrappers.append((role, flatten))

        after_inputs = input_sources(material, output)
        require(after_inputs.get("Global Height") is None, "Global Height became connected")
        edge_records = []
        for role, flatten in wrappers:
            input_name = role + " Height"
            flatten_inputs = input_sources(material, flatten)
            require(flatten_inputs.get("Elevation") == original_sources[role], "Flatten Elevation mismatch: " + role)
            require(flatten_inputs.get("Water Level") is None, "Water Level must remain unconnected: " + role)
            require(flatten_inputs.get("Threshold") == threshold, "Shared Threshold mismatch: " + role)
            require(after_inputs.get(input_name) == flatten, "Output wrapper mismatch: " + role)
            require(abs(float(flatten.get_editor_property("water_level"))) <= 1e-9, "Water default drift: " + role)
            require(abs(float(flatten.get_editor_property("threshold")) - 0.005) <= 1e-9, "Threshold default drift: " + role)
            edge_records.append({
                "role": role,
                "existing_source": original_sources[role].get_name(),
        ...
        for record in edge_records:
            require(any(edge["source"] == record["existing_source"] and edge["target"] == record["flatten_node"] and edge["input"] == "Elevation" for edge in edges), "Missing source edge: " + record["role"])
            require(any(edge["source"] == threshold.get_name() and edge["target"] == record["flatten_node"] and edge["input"] == "Threshold" for edge in edges), "Missing threshold edge: " + record["role"])
            require(any(edge["source"] == record["flatten_node"] and edge["target"] == output.get_name() and edge["input"] == record["output_input"] for edge in edges), "Missing output edge: " + record["role"])

        require(abs(float(threshold.get_editor_property("default_value")) - 0.005) <= 1e-9, "Shared threshold default drift")
        ...
        report.update({
            "status": "PASS_PROFILE_V1_SIX_PER_BIOME_SHORELINE_FLATTEN_WRAPPERS_PERSISTED_UNBOUND",
            ...
            "expression_count_after": len(list(unreal.MaterialEditingLibrary.get_material_expressions(material))),
            "global_height_connected": False,
            "shared_threshold_node": threshold.get_name(),
            "shared_threshold_parameter": "ShorelineFlattenThreshold",
            "shared_threshold_default": 0.005,
            "wrappers": edge_records,
            "wrapper_count": len(edge_records),
            "save_called": save_called,
            "regeneration_called": False,
            "home_map_saved": False,
            "profile_bound_to_home": False,
            "dirty_packages_after": dirty_packages(),
            "behavioral_boundary": "per-biome preblend flatten; not claimed equivalent to post-blend flatten at transitions",
            "next_safe_action": "Fresh-process reload Entry plus only ProfileV1 generation and verify one scalar, six native wrappers, exact defaults/wiring, changed generation hash and unchanged home/ProfileV1/protected hashes. Do not save or regenerate.",
        })
        ...
        unreal.log("REDMMO_PPG_PROFILE_V1_PER_BIOME_SHORELINE_FLATTEN " + report["status"])
        schedule_exit()
