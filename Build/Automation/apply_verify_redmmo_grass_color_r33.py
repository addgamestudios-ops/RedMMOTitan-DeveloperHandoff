"""Rollback-backed R33 grass color restraint and D3D12 proof."""
...
ROLLBACK_MATERIAL_SHAS = [
    "DD8E4BEE5ECB06FE126E1662E1D35475AC0441BB4FD2015BD1BF59C44952C82E",
    "9EC5A7261F8C9606931DD596A4F71F7ADD8D746F6C5C5055523E2951939E6CC1",
]
APPROVED_GRASS = {
...
ROLLBACK = Path(r"D:\RedMMOTitanWindowsData\Rollback\BeforeGrassColor_R33_20260805T1335Z")
ROLLBACK_MANIFEST = ROLLBACK / "pre_mutation_manifest.json"
ROLLBACK_MANIFEST_SHA = "2A18F19610BACB7CCC5467CB96180F319A2D4017FF7F23FA143CE969F3C69929"
PROTECTED = {
...
DIAG = Path(r"D:\RedMMOTitanWindowsData\Diagnostics\RedMMO_GrassColor_R33C_20260805T1400Z")
RESULT = DIAG / "result.json"
UNLIT = DIAG / "grass_only_unlit_r33.png"
NORMAL = DIAG / "player_scale_normal_lit_r33.png"

COLORS = [
    unreal.LinearColor(0.035, 0.140, 0.018, 1.0),
    unreal.LinearColor(0.045, 0.180, 0.024, 1.0),
]
HIGHLIGHTS = [
    unreal.LinearColor(0.080, 0.220, 0.035, 1.0),
    unreal.LinearColor(0.110, 0.280, 0.050, 1.0),
]
SCALARS = {
    "GrassColor_Custom_Enable": 1.0,
    "GrassColor_Effects_Enable": 1.0,
    "GrassColor_FromLandscape_Amount": 0.0,
    "GrassColor_Intensity": 0.82,
    "GrassColor_Contrast": 0.96,
    "GrassColor_Saturation": -0.05,
    "GrassColor_MinValue": 0.72,
    "GrassColor_MaxValue": 0.95,
}
...
def set_toggle(instance, name, value, record):
    ...
    if name in scalar_names:
        ...
        changed = abs(before - float(value)) > 0.0001
        if changed:
...
class R33:
    def __init__(self):
        ...
        self.report = {
            "schema": "redmmo.grass_color.apply_verify.r33.v1",
            "status": "RUNNING",
            "started_utc": now(),
    ...
    def set_phase(self, phase):
        ...
        unreal.log("REDMMO_R33_PHASE " + phase)
    ...
    def authenticate(self):
        ...
        require(not RESULT.exists() and not UNLIT.exists() and not NORMAL.exists(), "R33 no-clobber failed")
        ...
        for index, expected in enumerate(ROLLBACK_MATERIAL_SHAS):
    ...
    def apply_material_correction(self):
        ...
        require(R32_PARENT_FILE.is_file() and unreal.EditorAssetLibrary.does_asset_exist(R32_PARENT), "required R32 parent missing")
        require(sha256(R32_PARENT_FILE) == "2BD2B8DD41C611CF1250F1A39C40D3B4A7C47B5EB71ECD1497732546B80F0210", "required R32 parent drift")
        parent = unreal.load_asset(R32_PARENT)
        require(parent is not None and asset_path(parent) == R32_PARENT, "required R32 parent load failed")
        self.report["reused_r32_parent"] = True
        compile_errors = []
        ...
        for index, package in enumerate(GRASS_MATERIALS):
            ...
            changed = asset_path(instance.get_editor_property("parent")) != R32_PARENT
            if changed:
            ...
            for name, value in SCALARS.items():
                changed = set_toggle(instance, name, value, record["parameters"]) or changed
            ...
            color_changed = max(abs(a - b) for a, b in zip(color(before_color), color(COLORS[index]))) > 0.0001
            highlight_changed = max(abs(a - b) for a, b in zip(color(before_highlight), color(HIGHLIGHTS[index]))) > 0.0001
            if color_changed:
            ...
            changed = changed or color_changed or highlight_changed
            ...
            require((record["after_sha256"] != record["before_sha256"]) == changed, "grass material serialization postcondition failed: " + package)
            record["serialized_change"] = changed
            changes.append(record)
        self.report["color_correction"] = {
    ...
    def finish(self):
        ...
        self.report.update({
            "status": "PASS_R33_GRASS_COLOR_SAVED_AND_D3D12_CAPTURED_PENDING_PIXEL_REVIEW",
...
try:
    ...
    DIAG.mkdir(parents=True, exist_ok=True)
    atomic_json(RESULT, {"schema": "redmmo.grass_color.apply_verify.r33.v1", "status": "FAIL", "error": str(bootstrap_error), "traceback": traceback.format_exc(), "completed_utc": now()})
    unreal.SystemLibrary.quit_editor()
