"""Start and attest visible R10N PIE at the untouched actual PlayerStart."""
...
RESULT = DIAG / "start_r10n_actual_playerstart_pie_result.json"
PNG = DIAG / "RedMMO_Home_R10N_actual_PlayerStart_PIE_1920x1080.png"
EXPECTED_LOG = DIAG / "start_r10n_actual_playerstart_pie.log"
VERIFY = DIAG / "verify_r10n_fresh_reload_result.json"
PROTECTED = {
...
class Session:
    def __init__(self):
        ...
        self.report = {
            "schema": "redmmo.ppg_home_presentation.r10n.actual_playerstart_pie.v1",
            "status": "RUNNING",
            "started_utc": now(),
            "map": HOME_MAP,
            "home_map_sha256": EXPECTED_HOME,
            "evidence_class": "real_d3d12_pie_actual_untouched_playerstart",
    ...
    def start(self):
        ...
        unreal.log("REDMMO_R10N_PLAYERSTART_VALIDATION_BOOTSTRAPPED")
    ...
    def phase(self, state):
        ...
        self.frames = 0
        self.ready_frames = 0
        unreal.log("REDMMO_R10N_PLAYERSTART_PHASE " + state)
    ...
    def editor_contract(self):
        ...
        require(verify.get("status") == "PASS_FRESH_RELOAD_AND_MAPCHECK_PENDING_ACTUAL_PLAYERSTART_REAL_PIE", "R10N reload evidence missing")
        ...
        require(self.editor is not None and self.level is not None, "Editor subsystems unavailable")
        ...
        require(asset_path(spawners[0].get_editor_property("planet_data")) == TARGET_PLANET, "R10N PlanetData is not bound")
        ...
        self.report["editor_contract"] = {
            ...
            "spawner": spawners[0].get_path_name(),
            "planet_data": TARGET_PLANET,
            "player_start": starts[0].get_path_name(),
        ...
        unreal.log("REDMMO_R10N_PLAYERSTART_PIE_BEGIN")
    ...
    def find_pie_contract(self):
        ...
        require(start_dot >= 0.999999, "PIE PlayerStart radial direction drift")
        require(pawn_dot >= 0.999999, "PIE pawn is not at actual PlayerStart radial region")
        require(pawn_to_start <= 1000.0, "PIE pawn is more than 10 m from PlayerStart")

        view_location = pawn_location
        ...
        view_dot = dot(view_direction, self.editor_direction)
        require(view_dot >= 0.999999, "PIE player viewpoint is not at actual PlayerStart radial region")
        ...
        self.report["pie_playerstart_contract"] = {
            ...
            "game_mode": game_mode.get_class().get_name() if game_mode is not None else None,
            "spawner": spawners[0].get_path_name(),
            "player_start": starts[0].get_path_name(),
            "player_start_location": vec(start_location),
    ...
    def generation_status(self):
        ...
        return {
            ...
            "progress": progress,
            "ready": "COMPLETE" in phase.upper() and not generating and progress >= 0.999,
        }
    ...
    def collect_foliage_components(self):
        ...
        require(seen_grass == TARGET_GRASS_MESHES, "Both R10N grass mesh components are not present in actual PIE")
        self.report["pie_foliage"] = {
            "foliage_actor": foliage_actor.get_path_name(),
            "static_mesh_component_count": len(components),
            "r10n_grass_meshes_present": sorted(seen_grass),
            "components": records,
    ...
    def issue_screenshot(self):
        ...
        unreal.SystemLibrary.execute_console_command(self.pie_world, "DisableAllScreenMessages")
        unreal.SystemLibrary.execute_console_command(
            self.pie_world,
            'HighResShot filename="{}" 1920x1080'.format(str(PNG).replace("\\", "/")),
        )
        unreal.log("REDMMO_R10N_PLAYERSTART_SCREENSHOT_ISSUED")
    ...
    def finish(self):
        ...
        unreal.log("REDMMO_R10N_PLAYERSTART_SCREENSHOT_READY")
        time.sleep(0.25)
        ...
        generated = [int(item[0]) for item in overflows]
        retained = sorted(set(int(item[1]) for item in overflows))
        self.report.update(
            {
                "status": (
                    "REAL_GPU_ACTUAL_PLAYERSTART_PIE_READY_WITH_PER_CHUNK_FOLIAGE_CAP_BLOCKER"
                    if overflows
                    else "PASS_REAL_GPU_ACTUAL_PLAYERSTART_PIE_READY_PENDING_HUMAN_REVIEW"
                ),
                "completed_utc": now(),
                "generation": {
                    "time_ms": float(generations[-1][0]),
                    "chunks": int(generations[-1][1]),
                    "foliage_overflow_count": len(overflows),
                    "foliage_generated_min": min(generated) if generated else 0,
                    "foliage_generated_max": max(generated) if generated else 0,
                    "foliage_retained_values": retained,
                },
                "screenshot": {
                    "path": str(PNG),
                    "sha256": sha256(PNG),
        ...
        unreal.log("REDMMO_R10N_PLAYERSTART_PIE_READY")
        self.stop_callback()
    ...
    def fail(self, error):
        ...
        if not RESULT.exists():
            write_result(self.report)
        unreal.log_error("REDMMO_R10N_PLAYERSTART_PIE_FAIL " + str(error))
        self.stop_callback()
...
try:
    ...
    unreal.log_error("REDMMO_R10N_PLAYERSTART_PIE_BOOTSTRAP_FAIL " + str(bootstrap_error))
