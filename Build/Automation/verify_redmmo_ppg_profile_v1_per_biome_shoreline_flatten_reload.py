"""Fresh-process readback of ProfileV1 per-biome shoreline flatten wiring."""
...
RESULT = DIAG / "verify_redmmo_ppg_profile_v1_per_biome_shoreline_flatten_reload_result.json"
...
def main():
    report = {
        "schema": "redmmo.ppg_profile_v1.per_biome_shoreline_flatten.reload.v1",
        "status": "RUNNING",
        "started_utc": now(),
    ...
    try:
        ...
        require(not dirty_packages(), "Dirty packages before fresh reload")
        ...
        output = outputs[0]
        output_inputs = input_sources(material, output)
        ...
        for role, expected_source_name in ROLE_SOURCES:
            ...
            require(inputs.get("Elevation") is not None and inputs["Elevation"].get_name() == expected_source_name, "Elevation source mismatch: " + role)
            ...
            output_input = role + " Height"
            require(output_inputs.get(output_input) == flatten, "Output wrapper mismatch: " + role)
            wrappers.append({
                "role": role,
                "existing_source": expected_source_name,
                "flatten_node": flatten.get_name(),
                "identity": identity,
                "output_input": output_input,
            })

        require(not dirty_packages(), "Fresh reload dirtied packages")
        require(sha256(HOME_FILE) == EXPECTED_HOME, "Home map changed")
        require(sha256(GENERATION_FILE) == EXPECTED_GENERATION, "Generation changed during reload")
        for path, expected_hash in EXPECTED_UNCHANGED.items():
            require(sha256(asset_file(path)) == expected_hash, "ProfileV1 asset changed during reload: " + path)
        for path, expected_hash in PROTECTED.items():
            require(sha256(path) == expected_hash, "Protected asset changed during reload: " + str(path))

        report.update({
            "status": "PASS_PROFILE_V1_SIX_PER_BIOME_SHORELINE_FLATTEN_WRAPPERS_FRESH_RELOAD_UNBOUND",
            ...
            "behavioral_boundary": "per-biome preblend flatten persisted; no post-blend equivalence or visual claim",
            "next_safe_action": "Map the persisted ShorelineFlattenThreshold identity into the project-owned PPG profile adapter/control surface without binding or regenerating the home map; retain ProfileV1 unbound.",
        })
        ...
        unreal.log("REDMMO_PPG_PROFILE_V1_PER_BIOME_SHORELINE_FLATTEN_RELOAD " + report["status"])
        schedule_exit()
