"""R41 untouched default-render proof for the project-owned PPG grass refresh."""
...
HEADER_SHA = "904B46C063F74A01D7540E80844C2FA19D2A6CD5C1EF246D50B68201D06A5912"
SOURCE_FILE = PROJECT.parent / r"Source\RedMMO\Private\RedPPGGameplayGameMode.cpp"
SOURCE_SHA = "706792C1DCA23B29A5B7B0D53C279494C888D69FD9373B6363579B9BB6203C1F"
BINARY_FILE = PROJECT.parent / r"Binaries\Win64\UnrealEditor-RedMMO.dll"
BINARY_SHA = "B6664A1E64D4213A74CD2EAC1B65462F14D27E99CC5B40B47270FDA417D23DC1"
GRASS_FILES = {
...
DIAG = Path(r"D:\RedMMOTitanWindowsData\Diagnostics\RedMMO_GrassRenderRefresh_R41_20260805T1525Z")
RESULT = DIAG / "result.json"
WIREFRAME = DIAG / "default_wireframe_after_refresh_r41.png"
LIT = DIAG / "default_lit_after_refresh_r41.png"
...
class R41:
    def __init__(self):
        ...
        self.grass = []
        self.report = {
            "schema": "redmmo.grass_render_refresh.verify.r41.v1",
            "status": "RUNNING",
            "started_utc": now(),
            "evidence_class": "build_plus_fresh_reload_pie_plus_real_gpu",
    ...
    def set_phase(self, phase):
        ...
        unreal.log("REDMMO_R41_PHASE " + phase)
    ...
    def authenticate(self):
        ...
        require(not RESULT.exists() and not WIREFRAME.exists() and not LIT.exists(), "R41 no-clobber failed")
        checks = (
            ...
            (HEADER_FILE, HEADER_SHA), (SOURCE_FILE, SOURCE_SHA), (BINARY_FILE, BINARY_SHA),
        )
    ...
    def prepare_capture(self):
        ...
        summary = component_summary(self.grass)
        require(summary["instances"] == 2218356, "approved instance-count drift")
        require(summary["visible_true"] == 196 and summary["hidden_in_game_true"] == 0, "approved visibility drift")
        require(summary["registered"] == 196 and summary["instance_data_ready"] == 196, "approved runtime-readiness drift")
        self.report["state_before_any_capture"] = summary
    ...
    def finish_captures(self):
        require(WIREFRAME.is_file() and LIT.is_file(), "R41 capture missing")
        self.report["state_after_captures"] = component_summary(self.grass)
        self.report["captures"] = {
            "untouched_default_wireframe": {"path": str(WIREFRAME), "bytes": WIREFRAME.stat().st_size, "sha256": sha256(WIREFRAME)},
            "untouched_default_lit": {"path": str(LIT), "bytes": LIT.stat().st_size, "sha256": sha256(LIT)},
        }
    ...
    def finish(self):
        ...
        checks = (
            ...
            (SURFACE_FILE, SURFACE_SHA), (PARENT_FILE, PARENT_SHA), (HEADER_FILE, HEADER_SHA),
            (SOURCE_FILE, SOURCE_SHA), (BINARY_FILE, BINARY_SHA),
        )
        for path, expected in checks:
        ...
        self.report.update({
            "status": "PASS_R41_UNTOUCHED_DEFAULT_CAPTURES_PENDING_PIXEL_VISUAL_REVIEW",
            "completed_utc": now(),
            ...
            "visibility_cycle_called": False,
            "component_property_mutation_called": False,
            "claim_limit": "Fresh-reload exact-camera PIE and real-GPU proof only; no map/content save, visual acceptance, package, replication, or multiplayer claim.",
        })
        atomic_json(RESULT, self.report)
        unreal.log("REDMMO_R41_PASS")
        self.phase = "DONE"
    ...
    def fail(self, error):
        ...
        if not RESULT.exists():
            atomic_json(RESULT, self.report)
        unreal.log_error("REDMMO_R41_FAIL " + str(error))
    ...
    def tick(self, _delta):
        try:
            ...
            if self.phase == "PREPARE":
                ...
                require(elapsed <= 30.0, "project-owned refresh settle timeout")
                if elapsed >= 12.0:
                    self.prepare_capture()
            elif self.phase == "SETTLE_WIREFRAME":
                require(elapsed <= 15.0, "wireframe settle timeout")
                if elapsed >= 3.0:
                    self.request_capture(WIREFRAME, "WAIT_WIREFRAME")
            elif self.phase == "WAIT_WIREFRAME":
                require(elapsed <= 25.0, "wireframe capture timeout")
                if WIREFRAME.is_file() and WIREFRAME.stat().st_size > 0:
                    ...
                    self.set_phase("SETTLE_LIT")
            elif self.phase == "SETTLE_LIT":
                require(elapsed <= 15.0, "lit settle timeout")
                if elapsed >= 4.0:
                    self.request_capture(LIT, "WAIT_LIT")
            elif self.phase == "WAIT_LIT":
                require(elapsed <= 25.0, "lit capture timeout")
...
try:
    ...
    atomic_json(RESULT, {"schema": "redmmo.grass_render_refresh.verify.r41.v1", "status": "FAIL", "completed_utc": now(), "error": str(bootstrap_error), "traceback": traceback.format_exc()})
    unreal.SystemLibrary.quit_editor()
