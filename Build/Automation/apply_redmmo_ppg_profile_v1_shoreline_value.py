"""Persist the authenticated ProfileV1 shoreline scalar without binding or regeneration.

This transaction may change only the default value of the single tagged
ShorelineFlattenThreshold scalar in the unbound ProfileV1 generation material.
The six exact native consumers, home map, companion profile assets, and protected
production maps are validated before and after the save.
"""
...
ROLLBACK_FILE = Path(r"D:\RedMMOTitanWindowsData\Rollback\BeforeRedPPGProfileV1ShorelineValue_20260805T0614Z\M_PPG_ProfileV1_Generation.uasset")
DIAG = Path(r"D:\RedMMOTitanWindowsData\Diagnostics\RedMMO_PPG_ProfileV1ShorelineValue_20260805_0614")
RESULT = DIAG / "apply_redmmo_ppg_profile_v1_shoreline_value_result.json"
...
EXPECTED_GENERATION = "29BB81A7B842956F0246AB921CA15E5F410DC74A09635C69EA6491295032E634"
EXPECTED_OLD_VALUE = 0.005
...
def validate_graph(material, expected_value):
    ...
    require(abs(float(threshold.get_editor_property("default_value")) - expected_value) <= 1e-9,
            "Shared threshold default drift")
    ...
    for role, expected_source_name in ROLE_SOURCES:
        identity = "RedProfile.ShorelineFlatten." + role
        flatten = by_desc.get(identity)
        require(flatten is not None, "Missing flatten identity: " + role)
        inputs = input_sources(material, flatten)
        require(inputs.get("Elevation") is not None and inputs["Elevation"].get_name() == expected_source_name,
        ...
        require(abs(float(flatten.get_editor_property("water_level"))) <= 1e-9,
                "Water default drift: " + role)
        ...
        wrappers.append({"role": role, "source": expected_source_name, "node": flatten.get_name()})
    return threshold, wrappers, len(nodes)
...
def main():
    report = {
        "schema": "redmmo.ppg_profile_v1.shoreline_value.apply.v1",
        "status": "RUNNING",
        "started_utc": now(),
        "evidence_class": "editor_automation_bounded_asset_mutation",
    }
    try:
        ...
        require(not dirty_packages(), "Dirty packages before transaction")
        require(GENERATION_FILE.is_file() and sha256(GENERATION_FILE) == EXPECTED_GENERATION,
                "Generation preimage hash drift")
        require(ROLLBACK_FILE.is_file() and sha256(ROLLBACK_FILE) == EXPECTED_GENERATION,
                "Rollback preimage missing or mismatched")
        ...
        threshold, wrappers, expression_count = validate_graph(material, EXPECTED_OLD_VALUE)
        threshold.set_editor_property("default_value", REQUESTED_VALUE)
        validate_graph(material, REQUESTED_VALUE)
        require(dirty_packages() == [GENERATION], "Unexpected dirty package set before save: " + repr(dirty_packages()))
        require(unreal.EditorAssetLibrary.save_loaded_asset(material, only_if_is_dirty=False), "Generation material save failed")
        require(not dirty_packages(), "Dirty packages remain after shoreline value save")
        ...
        report.update({
            "status": "PASS_PROFILE_V1_SHORELINE_VALUE_PERSISTED_UNBOUND",
            ...
            "scalar_identity": "RedProfile.ShorelineFlattenThreshold",
            "scalar_default_before": EXPECTED_OLD_VALUE,
            "scalar_default_after": REQUESTED_VALUE,
            "save_called": True,
            "regeneration_called": False,
            "home_map_saved": False,
            "profile_bound_to_home": False,
            "dirty_packages_after": dirty_packages(),
            "next_safe_action": "Fresh-process read-only reload verification of the exact scalar value, graph topology, and protected hashes.",
        })
    except Exception as exc:
        report["status"] = "FAIL"
        report["error"] = str(exc)
        report["traceback"] = traceback.format_exc()
        report["generation_hash_observed"] = sha256(GENERATION_FILE) if GENERATION_FILE.is_file() else None
        report["rollback_required"] = report["generation_hash_observed"] not in (None, EXPECTED_GENERATION)
    finally:
        report["completed_utc"] = now()
        write_json_exclusive(RESULT, report)
        unreal.log("REDMMO_PPG_PROFILE_V1_SHORELINE_VALUE " + report["status"])
        schedule_exit()
